function [ TPUPParameters_fixedTemporalParameters ] = fitTPUPToSubjectAverageResponses_fixedTemporalParameters(goodSubjects, averageResponsePerSubject, TPUPParameters, dropboxAnalysisDir, varargin)
...
for session = 1:length(goodSubjects) % loop over sessions
    for ss = 1:length(goodSubjects{session}.ID) % loop over subjects
        for stimulus = 1:length(p.Results.stimulusLabels) % loop over stimuli
            ...
            % determine what the locked temporal parameters for be
            % average temporal parameters for mel and lms stimulation
            % within a subject, same thing for blue and red within a
            % subject
            ...
            % save out the plot
            outDir = fullfile(dropboxAnalysisDir,'pupilPIPRAnalysis/TPUP/modelFits_fixedTemporalParameters/', p.Results.stimulusLabels{stimulus}, num2str(session));
            if ~exist(outDir, 'dir')
                mkdir(outDir);
            end
            ...
            TPUPParameters_fixedTemporalParameters{session}.(p.Results.stimulusLabels{stimulus}).transientAmplitude(ss) = paramsFit.paramMainMatrix(4);
            TPUPParameters_fixedTemporalParameters{session}.(p.Results.stimulusLabels{stimulus}).sustainedAmplitude(ss) = paramsFit.paramMainMatrix(5);
            TPUPParameters_fixedTemporalParameters{session}.(p.Results.stimulusLabels{stimulus}).persistentAmplitude(ss) = paramsFit.paramMainMatrix(6);
            TPUPParameters_fixedTemporalParameters{session}.(p.Results.stimulusLabels{stimulus}).delay(ss) = paramsFit.paramMainMatrix(1);
            TPUPParameters_fixedTemporalParameters{session}.(p.Results.stimulusLabels{stimulus}).gammaTau(ss) = paramsFit.paramMainMatrix(2);
            TPUPParameters_fixedTemporalParameters{session}.(p.Results.stimulusLabels{stimulus}).exponentialTau(ss) = paramsFit.paramMainMatrix(3);
            TPUPParameters_fixedTemporalParameters{session}.(p.Results.stimulusLabels{stimulus}).rSquared(ss) = rSquared;
