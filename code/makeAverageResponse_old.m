function [piprCombined, averageMelCombined, averageLMSCombined, averageBlueCombined, averageRedCombined, semLMS, semMel, semBlue, semRed] = makeAverageResponse_old(goodSubjects, dropboxAnalysisDir)
...
for session = 1:2;
    for ss = 1:size(goodSubjects{session}{1},1);
        ...
        ylabel('Percent Change (%)');
        ylim([-60 20]);
        outDir = fullfile(dropboxAnalysisDir,'pupilPIPRAnalysis/dataOverview/averageResponse/LMS', num2str(session));
        if ~exist(outDir, 'dir')
            mkdir(outDir);
        end
        saveas(plotFig, fullfile(outDir, [subject, '.png']), 'png');
        ...
        ylabel('Percent Change (%)');
        ylim([-60 20]);
        outDir = fullfile(dropboxAnalysisDir,'pupilPIPRAnalysis/dataOverview/averageResponse/mel', num2str(session));
        if ~exist(outDir, 'dir')
            mkdir(outDir);
        end
        saveas(plotFig, fullfile(outDir, [subject, '.png']), 'png');
